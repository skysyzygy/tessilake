...
setleftjoin <- function(...) {
  suppressMessages(tessilake::setleftjoin(...))
}
...
merge.data.table <- function(...) {
  setkey(data.table::merge.data.table(...), NULL)
}
...
test_that("update_table requires that primaryKeys and dateColumn refer to columns in from and to", {
  from = data.table(x=c(1,2,3))
  to = data.table(y=c(1,2,3))
  expect_error(update_table(from,to,primaryKeys=x),"colnames")
  expect_error(update_table(from,to,dateColumn=y),"colnames")
})

test_that("update_table works with tidy-selected columns in primaryKeys and dateColumn", {
    from = data.table(x=c(1,2,3),y=c(4,5,6))
    to = copy(from)
    expect_equal(update_table(from,to,primaryKeys=x),from)
    expect_equal(update_table(from,to,primaryKeys=c(x)),from)
    expect_equal(update_table(from,to,dateColumn="x"),from)
    expect_equal(update_table(from,to,dateColumn=c("x")),from)
    expect_equal(update_table(from,to,primaryKeys=c(x,y)),from)
    expect_equal(update_table(from,to,dateColumn=c("x","y")),from)
    expect_equal(update_table(from,to,primaryKeys=c(x,"y")),from)
    expect_equal(update_table(from,to,dateColumn=c("x",y)),from)
})
